...
// use the closure compiler to get minify the js: https://closure-compiler.appspot.com/home
...
function resize_divs() {
  jQuery('.flip-card').each(function () {
    ...
    if (img.length != 0) {
      let height = Math.floor(img[0].clientHeight);
...
function resize_single_div(object) {
  object.parentElement.parentElement.parentElement.style.height =
    Math.floor(object.clientHeight) + 'px';
}
...
function mobile_event_div() {
  ...
  if (window_width < 768 && window.global_width_flag == false) {
    jQuery('.flip-card-back-content').each(function () {
      ...
      let num_removed = jQuery(this).children('p').length * -1;
      jQuery(this)
        .children('.wp-block-buttons')
        .css('margin-top', num_removed + 'px');
...
if (
  ...
  window.location.pathname == '/sponsors/our-partners/'
) {
